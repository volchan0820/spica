<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <title>Hair atelier spica | Concept | 浦添市経塚のカット技術と薬剤へのこだわり</title>
    <meta name="description" content="浦添市経塚のhair atelier spicaのコンセプトページ。再現性の高いカット技術や髪と頭皮に優しい弱酸性薬剤へのこだわりで、美しさと扱いやすさを両立。細部まで丁寧に施術することを大切にしています。">

    <!-- CSS -->
    <?php echo $this->element('head_css'); ?>
    <!-- 使用フォント -->
    <?php echo $this->element('head_fonts'); ?>
    <!-- JavaScript -->
    <?php echo $this->element('head_scripts'); ?>
</head>
<body>
    <?php echo $this->element('spica_header'); ?>

    <div class="spica-page-img">
        <img src="/assets/images/concept/main/concept-main.jpeg" alt="hair atelier spica">
    </div>

    <h1 class="spica-title fade-up">CONCEPT</h1>

    <div class="contents-wrapper">
        <main class="concept-section">
            <!-- プライベートサロン -->
            <section class="concept-block fade-up">
                <div class="concept-text">
                    <h2>- Private Salon -</h2>
                    <p>
                        店内一席のみの完全プライベートサロン ― <br>
                        周囲を気にせず、静かにお過ごしいただけます。<br>
                        移動のいらない最新のフルフラットチェアを採用し、心からくつろげる上質な空間をご用意しています。
                    </p>
                </div>
                <div class="concept-image">
                    <img src="/assets/images/concept/main/concept-privatesalon.jpeg" alt="完全プライベートサロン hair atelier spica の店内">
                </div>
            </section>
            <div class="concept-btn-1">
                <a href="/pages/private_salon_details/" class="btnBasic">
                    <div class="btnBasic__label">
                        <span class="typo">V</span><span class="typo">I</span><span class="typo">E</span><span class="typo">W</span>&ensp;
                        <span class="typo">M</span><span class="typo">O</span><span class="typo">R</span><span class="typo">E</span>
                        <div class="btnBasic__bg"></div>
                    </div>
                    <div class="arrow more__arrow">
                        <div class="arrow__line arrow__line--01"></div>
                        <div class="arrow__line arrow__line--02"></div>
                    </div>
                </a>
            </div>
            <!-- カウンセリング -->
            <section class="concept-block reverse fade-up">
                <div class="concept-text">
                    <h2>- Counseling -</h2>
                    <p>
                        spicaではカウンセリングを何より大切にしています。<br>
                        お悩みやご要望を丁寧に伺い、髪質や骨格を見極めたうえで、日常に寄り添う最適なヘアスタイルをご提案いたします。
                    </p>
                </div>
                <div class="concept-image">
                    <img src="/assets/images/concept/main/concept-counseling.jpeg" alt="初めての方も安心できる丁寧なカウンセリング hair atelier spica">
                </div>
            </section>
            <!-- カット -->
            <section class="concept-block fade-up">
                <div class="concept-text">
                    <h2>- Cut -</h2>
                    <p>
                        カットは仕上がりだけでなく、日々の扱いやすさを左右します。<br>
                        髪質や骨格を的確に捉え、計算されたカットで、ご自宅でも美しく再現できるスタイルを追求しています。
                    </p>
                </div>
                <div class="concept-image">
                    <img src="/assets/images/concept/main/concept-cut.jpeg" alt="一人ひとりに似合わせたカットを行う hair atelier spica">
                </div>
            </section>
            <!-- ヘアケア -->
            <section class="concept-block reverse fade-up">
                <div class="concept-text">
                    <h2>- Hair Care -</h2>
                    <p>
                        マイクロバブルによるやさしいクレンジングで、毛穴や頭皮の奥まで丁寧に洗浄します。<br>
                        全メニューにトリートメント工程を組み込み、髪と頭皮への負担を最小限に抑えています。
                    </p>
                </div>
                <div class="concept-image">
                    <img src="/assets/images/concept/main/concept-haircare.jpeg" alt="髪のダメージを抑える丁寧なヘアケア施術 hair atelier spica">
                </div>
            </section>
            <div class="concept-btn-2-reverse">
                <a href="/pages/hair_care_details/" class="btnBasic">
                    <div class="btnBasic__label">
                        <span class="typo">V</span><span class="typo">I</span><span class="typo">E</span><span class="typo">W</span>&ensp;
                        <span class="typo">M</span><span class="typo">O</span><span class="typo">R</span><span class="typo">E</span>
                        <div class="btnBasic__bg"></div>
                    </div>
                    <div class="arrow more__arrow">
                        <div class="arrow__line arrow__line--01"></div>
                        <div class="arrow__line arrow__line--02"></div>
                    </div>
                </a>
            </div>
            <!-- 代表挨拶 -->
            <section class="concept-block fade-up">
                <div class="concept-text">
                    <h2>- Owner's Message -</h2>
                    <p>
                        spicaが最も大切にしていることは、<br>
                        ”お客様一人ひとりに寄り添うこと” です。<br>
                        お悩みの本質を見極めるため、カウンセリングや似合わせ、ヘアケアに丁寧に向き合い、<br>
                        サロンを出た瞬間だけでなく、ご自宅でも美しさが続くヘアスタイルを心がけています。<br>
                        <br>
                        心からリラックスしていただけるよう、空間づくりにもこだわり、<br>
                        移動のない施術、他のお客様と顔を合わせない完全プライベート空間を実現しました。<br>
                        <br>
                        ”店内一席のみ” だからこそできる、全力で寄り添う施術を是非spicaでご体験いただけたらと思います。
                    </p>
                </div>
                <div class="concept-image">
                    <img src="/assets/images/concept/main/concept-ownersmessage.jpeg" alt="技術と想いを大切にする hair atelier spica オーナーのメッセージ写真">
                </div>
            </section>
            <div class="concept-btn-3">
                <a href="/pages/concept_staff/" class="btnBasic">
                    <div class="btnBasic__label">
                        <span class="typo">V</span><span class="typo">I</span><span class="typo">E</span><span class="typo">W</span>&ensp;
                        <span class="typo">M</span><span class="typo">O</span><span class="typo">R</span><span class="typo">E</span>
                        <div class="btnBasic__bg"></div>
                    </div>
                    <div class="arrow more__arrow">
                        <div class="arrow__line arrow__line--01"></div>
                        <div class="arrow__line arrow__line--02"></div>
                    </div>
                </a>
            </div>
            <!-- 3つの徹底事項 -->
            <?= $this->element('three_points') ?>
        </main>
        <!-- BACKボタン -->
        <?= $this->element('back_button_index') ?>
    </div>

    <!-- フッター -->
    <footer class="footer">
        <?php echo $this->element('spica_footer'); ?>
    </footer>

    <!-- ページトップボタン -->
	<a href="#" class="pagetop" id="pagetop"><span class="pagetop-text">PAGE TOP</span></a>
	<?php echo $this->element('page_top_script'); ?>

    <!-- レスポンシブ　ページトップボタン -->
    <a href="#top" id="pagetop-mobile" class="page-top"><img src="/img/pagetop.png" alt="ページトップへ"></a>
    <?php echo $this->element('page_top_mobile_script'); ?>

    <!-- フェードアップ -->
    <?= $this->element('fade_up_script') ?>
</body>
</html>
